...
const PricingCard = ({ tier }) => (
  <Card className="flex h-full flex-col gap-6 p-8 text-charcoal">
    <div className="flex items-center justify-between">
      <div>
        <h3 className="text-2xl font-heading">{tier.name}</h3>
        <p className="mt-2 text-sm text-charcoal/60">{tier.billing}</p>
      </div>
      {tier.badge ? (
    ...
    <div className="text-4xl font-heading text-charcoal">{tier.price}</div>
