...
const ProgramCard = ({ program }) => (
  <Card className="flex h-full flex-col overflow-hidden">
    <div className="relative h-56 w-full overflow-hidden">
      <img src={program.image} alt={program.name} className="h-full w-full object-cover transition duration-300 hover:scale-105" loading="lazy" />
    ...
    <div className="flex flex-1 flex-col gap-4 p-6">
      <div>
        <h3 className="text-2xl font-heading">{program.name}</h3>
        <p className="mt-1 text-sm uppercase tracking-wide text-charcoal/60">{program.duration}</p>
        {program.description && (
      ...
      {program.schedule && (
        <div className="rounded-lg bg-light-gray/50 p-3">
          <p className="text-xs font-semibold uppercase tracking-wide text-charcoal/60">Schedule</p>
          <p className="mt-1 text-sm text-charcoal/80">{program.schedule}</p>
        </div>
      )}
      ...
      <div className="mt-auto pt-4">
        <Button as="a" href="#book-trial">{program.cta || 'Join Now'}</Button>
      </div>
    </div>
  </Card>
)
